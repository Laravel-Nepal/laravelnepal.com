<footer class="py-20 border-t border-white/5 text-center" id="footer">
    <p class="text-[9px] text-zinc-600 uppercase tracking-[0.4em] mb-4">Laravel Nepal</p>
    <p class="max-w-2xl mx-auto text-xs text-zinc-500 px-6 uppercase leading-loose">
        Officially permitted by the Laravel team. <br />
        Not officially affiliated with or endorsed by Laravel.
    </p>
</footer>
